...
const { extractContentFromMessages } =
  await import("../ConversationExtractor.ts");
const {
...
const client = {
  user: { id: BOT_ID, username: "Lupos", displayAvatarURL: () => null },
};
...
function fakeMessage({
  ...
  return {
    ...
    author: {
      id: authorId,
      username: `user${authorId.slice(-2)}`,
      globalName: null,
    },
    member: null,
    ...
    stickers: new Collection(
      sticker ? [["s", { name: sticker.name, url: sticker.url }]] : [],
    ),
    embeds: [],
...
beforeEach(() => {
  ...
  vi.mocked(
    DiscordUtilityService.extractImageUrlsFromMessage,
  ).mockResolvedValue([]);
  vi.mocked(
    DiscordUtilityService.retrieveMessageReferenceFromMessage,
  ).mockClear();
...
describe("extractContentFromMessages — concurrency", () => {
  it("starts every message's link probes before any of them answers", async () => {
    const messages = [1, 2, 3, 4, 5].map((n) =>
      fakeMessage({ content: `link https://x.example.com/${n}` }),
    );
    ...
    vi.mocked(
      ...
      async (message: { id: string }) =>
        gates[messages.findIndex((m) => m.id === message.id)].promise,
    );
    ...
    expect(AIService.captionImages).toHaveBeenCalledWith(
      ["https://x.example.com/3"],
      {},
      "IMAGE",
    );
    expect(
      vi
        .mocked(AIService.captionImages)
        .mock.calls.filter((call) => call[2] === "IMAGE"),
    ).toHaveLength(1);
  ...
  it("starts a sticker's caption in the first pass, before the image captions answer", async () => {
    ...
    vi.mocked(
      DiscordUtilityService.extractImageUrlsFromMessage,
    ).mockImplementation(async (message: { id: string }) =>
      message.id === withImage.id
        ? ["https://cdn.discordapp.com/attachments/1/2/a.png"]
        : [],
    );
    const imageCaption = deferred<typeof noCaptions>();
    vi.mocked(AIService.captionImages).mockImplementation(
      async (_urls, _mongo, type) =>
        (type === "IMAGE" ? imageCaption.promise : noCaptions) as never,
    );
    const extraction = extractContentFromMessages(
      {
        message: withSticker as never,
        recentMessages: history([withImage, withSticker]),
      },
      {} as never,
      { windowSize: 50 },
    );
    await vi.waitFor(() =>
      expect(
        vi
          .mocked(AIService.captionImages)
          .mock.calls.some((call) => call[2] === "STICKER"),
      ).toBe(true),
    );
  ...
  it("never re-fetches a replied-to message that is in the fetched history", async () => {
    ...
    const inWindowReply = fakeMessage({
      content: "agreed",
      referenceId: filler[1].id,
    });
    const outOfWindowReply = fakeMessage({
      content: "what?",
      referenceId: quoted.id,
    });
    const { conversation } = await extractContentFromMessages(
      {
        message: outOfWindowReply as never,
    ...
    expect(conversation.at(-2)?.content).toContain(
      `<replying-to id="${filler[1].id}"`,
    );
    expect(conversation.at(-2)?.content).toContain('in-context="true"');
    expect(conversation.at(-1)?.content).toContain(
      `<replying-to id="${quoted.id}"`,
    );
  ...
  it("reports the window it chose and times its stages", async () => {
    ...
    await extractContentFromMessages(
      ...
      {
        windowSize: 2,
        onWindowSelected: (window) => selected.push(window.map((m) => m.id)),
        timings,
      },
    );
...
describe("buildAndGenerateReply — prep work", () => {
  ...
  it("starts the emoji captions up front, and keeps reference images before emojis", async () => {
    ...
    vi.mocked(AIService.captionImages).mockImplementation(
      async (urls, _mongo, type) => {
        smallCalls.push(urls as string[]);
        if (type !== "SMALL") return noCaptions as never;
        if ((urls as string[])[0] === imageUrl)
          return referenceCaption.promise as never;
        return {
          images: ["pog face"],
          imagesMap: new Map([
            ["h", { url: (urls as string[])[0], caption: "pog face" }],
          ]),
        } as never;
      },
    );
    const imagesCollection = new Collection([
      [
        trigger.id,
        new Collection([["h1", { url: imageUrl, caption: "long" }]]),
      ],
    ]);
    ...
    expect(smallCalls[0]).toEqual([
      "https://cdn.discordapp.com/emojis/111111111111111111.png",
    ]);
    referenceCaption.resolve({
      images: ["a cat"],
      imagesMap: new Map([["h2", { url: imageUrl, caption: "a cat" }]]),
    });
    await reply;
    ...
    const lastUser = sent.messages
      ...
      .at(-1);
    expect(lastUser.images).toEqual([
      imageUrl,
      "https://cdn.discordapp.com/emojis/111111111111111111.png",
    ]);
    expect(lastUser.content.indexOf("Attached image")).toBeLessThan(
      lastUser.content.indexOf("Emoji: pog"),
    );
  });
  ...
  it("logs one ⏱️ [prep] line with its stages right before calling /agent", async () => {
    ...
    vi.mocked(PrismService.generateAgentResponse).mockImplementation(
      async () => {
        expect(
          lines.filter((line) => line.startsWith("⏱️ [prep]")),
        ).toHaveLength(1);
        return {
          text: "ok",
          images: [],
    ...
    expect(line).toMatch(
      new RegExp(
        `^⏱️ \\[prep\\] ${trigger.id} fetch=1200 dossier=\\d+ refs=\\d+ avatars=\\d+ emoji=\\d+ prompt=\\d+ total=\\d+ sinceTrigger=\\d+$`,
      ),
    );
  });
});
...
describe("prefetchTriggerReferenceCaptions / prefetchRepliedImageCaption", () => {
  ...
  it("starts the SMALL captions of the trigger's image and video attachments, and its emojis", async () => {
    vi.mocked(
      DiscordUtilityService.extractImageUrlsFromMessage,
    ).mockResolvedValue(["https://cdn.discordapp.com/attachments/1/2/a.png"]);
    const trigger = fakeMessage({
      ...
      attachments: [
        ...
        {
          url: "https://cdn.discordapp.com/attachments/1/3/v.mp4",
    ...
    expect(AIService.captionImages).toHaveBeenCalledWith(
      [
        "https://cdn.discordapp.com/attachments/1/2/a.png",
        "https://cdn.discordapp.com/attachments/1/3/v.mp4",
      ],
      {},
      "SMALL",
    );
  ...
  it("starts nothing for a trigger without attachments or emojis (no wasted vision calls)", async () => {
    prefetchTriggerReferenceCaptions(
      fakeMessage({ content: "lupos, hi" }) as never,
      {} as never,
    );
    await settle();
  ...
  it("the replied-to image: as captioned in the extracted slice, else its first image attachment", async () => {
    const referenced = fakeMessage({
      attachments: [
        {
          url: "https://cdn.discordapp.com/attachments/1/9/r.png",
          proxyURL: "https://media.discordapp.net/attachments/1/9/r.png",
          contentType: "image/png",
        },
      ],
    });
    vi.mocked(
      DiscordUtilityService.extractImageUrlsFromMessage,
    ).mockResolvedValue(["https://cdn.discordapp.com/attachments/1/9/r.png"]);
    ...
    prefetchRepliedImageCaption({
      ...
      extractedMessageIds: new Set([referenced.id]),
      localMongo: {} as never,
    });
    await settle();
    expect(AIService.captionImages).toHaveBeenLastCalledWith(
      ["https://cdn.discordapp.com/attachments/1/9/r.png"],
      {},
      "SMALL",
    );
    ...
    await settle();
    expect(AIService.captionImages).toHaveBeenLastCalledWith(
      ["https://media.discordapp.net/attachments/1/9/r.png"],
      {},
      "SMALL",
    );
  });
  ...
  it("never throws, even on a message it can't read", () => {
    expect(() =>
      prefetchTriggerReferenceCaptions({} as never, {} as never),
    ).not.toThrow();
    expect(() =>
      prefetchRepliedImageCaption({
        message: {} as never,
